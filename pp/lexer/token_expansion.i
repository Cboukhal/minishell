# 1 "src/lexer/token_expansion.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "src/lexer/token_expansion.c" 2
# 13 "src/lexer/token_expansion.c"
...
# 14 "src/lexer/token_expansion.c" 2

_Bool is_next_char_valid(char c)
{
 if (c == '\0' || c == '$' || is_blank(c)
  || is_quote(c) || c == '=')
  return (0);
...
t_expan *get_token_expansion(char *lexeme, int length)
{
 ...
 expan_new = ((void*)0);
 expan_list = ((void*)0);
 while (i < length && (*__errno_location ()) == 0)
 {
  if (lexeme[i] == '\'')
