# 1 "src/parsing/command_generation.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "src/parsing/command_generation.c" 2
# 13 "src/parsing/command_generation.c"
...
# 14 "src/parsing/command_generation.c" 2
...
void extract_command(t_minishell *minishell, t_cmd **cmd, t_token **token)
{
 while (*token && (*token)->type != eof_token
  && (*__errno_location ()) == 0 && is_operator_token((*token)) == 0)
 {
  if (is_redirection((*token)->type) == 1)
